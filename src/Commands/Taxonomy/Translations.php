...
class Translations extends BaseTaxonomyImporter
{
    ...
    {
        $sourcePostHasTagIdsTranslated = collect(get_the_terms($sourcePostId, $taxonomyWP))->map(function ($sourceTag) use ($translatedPostId, $translatedLang, $sourcePostId) {
            if (!$sourceTag) {
                return null;
            }
            $translatedTerms = pll_get_term_translations($sourceTag->term_id);
            if (!isset($translatedTerms[$translatedLang])) {
                WP_CLI::warning('No term-translation found (sourcePostId,translatedPostId,term,lang): ' . $sourcePostId . ',' . $translatedPostId . ',' . $sourceTag->name . ',' . $translatedLang);
                return null;
            }
