...
class PermalinkHelper
{
    ...
    /**
     * @var $endpoints protect the endpoints from being vaped if a category/tag slug is set
     */
    protected $endpoints;
    ...
    public function parse_permalinks($post_link, \WP_Post $post, $leavename, $sample = false)
    {
        ...
        if (isset($post_type->rewrite[ static::$customPermalink ]) && ! empty($post_type->rewrite[ static::$customPermalink ])) {
    ...
    private function get_term_parents($id, $taxonomy, $link = false, $separator = '/', $nicename = false, $visited = array())
    {
        $chain = '';
